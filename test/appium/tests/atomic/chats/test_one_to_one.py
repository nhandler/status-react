...
class TestMessagesOneToOneChatMultiple(MultipleDeviceTestCase):
    ...
    @marks.testrail_id(6305)
    @marks.critical
    def test_send_audio_message_with_pn_check(self):
        self.create_drivers(2)
        device_1, device_2 = SignInView(self.drivers[0]), SignInView(self.drivers[1])
        ...
        device_2.open_notification_bar(1)
        ...
        if chat_2.audio_message_in_chat_timer.text != '00:05':
            self.errors.append("Listened 5 seconds but timer shows dirrent listened time in audio message")
